...
import com.mhealth.admin.dto.response.HospitalManagementResponseDto;
...
public class AgentUserService {
    ...
    public Object deleteAgentUser(Locale locale, Integer id) throws Exception {
        ...
        // Find the user
        Optional<Users> existingMarketingUser = usersRepository.findByUserIdAndType(id, UserType.Agentuser);
    ...
    public Object getAgentUser(Locale locale, Integer userId) {
        Response response = new Response();

        // Find the user
        Optional<Users> existingMarketingUser = usersRepository.findByUserIdAndType(userId, UserType.Agentuser);
        if (existingMarketingUser.isEmpty()) {
            response.setCode(Constants.CODE_O);
            response.setMessage(messageSource.getMessage(Messages.USER_NOT_FOUND, null, locale));
            response.setStatus(Status.FAILED);
