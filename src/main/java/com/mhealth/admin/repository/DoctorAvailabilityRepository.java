...
public interface DoctorAvailabilityRepository extends JpaRepository<DoctorAvailability, Integer>, JpaSpecificationExecutor<DoctorAvailability> {
    ...
    @Query(value = "SELECT u FROM DoctorAvailability u WHERE u.doctorId.userId = ?1 AND u.slotId.slotId IN ?2")
    List<DoctorAvailability> findBySlots(int userId, List<Integer> daySlots);
}